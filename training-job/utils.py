...
def enable_health_watch():
    cmd = "dcgmi health -g 1 -s mpi "
    try:
...
def run_health_checks(extra_commands: str = "", iteration: int = 10, interval: int = 10):
    health_check = "dcgmi health -g 1 -c -j"
    health_check += extra_commands
    for _ in range(iteration):
        ...
        if result_dict["body"]["Overall Health"]["value"] == "Healthy":
            print("Healthy node.. Waiting for {} seconds".format(interval))
        else:
            print("!!!!!!!!!! Warning: Unhealthy status: {}".format(json.dumps(result_dict)))
        time.sleep(interval)


def run_continous_monitoring(script_name: str):
    while True:
        result = run_command("ps -ef | grep '{}' | grep -v grep".format(script_name)).decode(
            "utf-8"
        )
        print("Result: ", result)
        if result.strip() == "":
            print("Script - {} has finished. Call stop monitoring".format(script_name))
            return
        else:
            print("Script - {}  is still running. Continuing the health checks".format(script_name))
            run_health_checks()
